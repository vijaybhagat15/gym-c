...
const Features = () => {
  const features = [
    {
      icon: <FiGift className="w-10 h-10 text-red-500" />,
      title: "1000+",
    ...
    {
      icon: <FiTruck className="w-10 h-10 text-red-500" />,
      title: "7 Days",
      subtitle: "Easy Return Policy",
    },
    {
      icon: <FiCheckCircle className="w-10 h-10 text-red-500" />,
      title: "Safe & Secure",
      subtitle: "Transport",
    },
  ];
    ...
    <div className="flex justify-center sm:gap-8 sm:py-8 py-4 gap-4 bg-white">
      {features.map((feature, index) => (
        ...
        >
          ...
          <p className="mt-2 sm:text-sm text-xs text-gray-500">{feature.subtitle}</p>
        </div>
      ))}
    </div>
  );
